...
func MakeUserPageData(transactions []*model.Transaction, credits []*model.Credit, findById func(int) string, backToBackSelector func()) fyne.CanvasObject {

	tabs := container.NewAppTabs(
		container.NewTabItem("ПЕРЕВОДЫ", MakeTransactionUserTab(transactions, findById, backToBackSelector)),
		container.NewTabItem("ЗАРПЛАТНЫЕ ПРОЕКТЫ", MakeSalaryUserTab()),
		container.NewTabItem("КРЕДИТЫ", MakeCreditUserTab(credits, findById, backToBackSelector)),
		container.NewTabItem("РАССРОЧКИ", MakeInstallmentUserTab()),
	)
	return tabs
}

func MakeTransactionUserTab(transactions []*model.Transaction, findById func(int) string, backToBackSelector func()) fyne.CanvasObject {
	...
	table := widget.NewTable(
		...
		func(id widget.TableCellID, cell fyne.CanvasObject) {
			...
			if id.Row == 0 {
				...
				case 5:
					myContainer.Add(widget.NewLabel("СТАТУС"))
				case 6:
					myContainer.Add(widget.NewButton("ВЕРНУТЬСЯ НАЗАД", backToBackSelector))
				}
				...
				for i, transaction := range transactions {
					if id.Row == i+1 {
						...
						case 5:
							myContainer.Add(widget.NewLabel(fmt.Sprintf(string(transaction.Status))))
						}
					}
...
func MakeSalaryUserTab() fyne.CanvasObject {
...
func MakeCreditUserTab(credits []*model.Credit, findById func(int) string, backToBackSelector func()) fyne.CanvasObject {
				...
				for i, credit := range credits {
					if id.Row == i+1 {
						...
						case 4:
							...
							myContainer.Add(widget.NewLabel(bank))
						case 5:
							myContainer.Add(widget.NewLabel(fmt.Sprintf(string(credit.Status))))
						}
					}
...
func MakeInstallmentUserTab() fyne.CanvasObject {
